...
def test_fresh_database_records_all_packaged_migrations(tmp_path):
    ...
    assert rows[2]["version"] == migrations[2].version == 3
    assert rows[2]["name"] == migrations[2].name == "analysis_worker_process"
    assert rows[2]["checksum"] == migrations[2].checksum
    assert rows[3]["version"] == migrations[3].version == 4
    assert rows[3]["name"] == migrations[3].name == "delivery_outbox_controls"
    assert rows[3]["checksum"] == migrations[3].checksum
    assert rows[4]["version"] == migrations[4].version == 5
    assert rows[4]["name"] == migrations[4].name == "quality_security_budget"
    assert rows[4]["checksum"] == migrations[4].checksum
    assert rows[5]["version"] == migrations[5].version == 6
    assert rows[5]["name"] == migrations[5].name == "operator_observability"
    assert rows[5]["checksum"] == migrations[5].checksum
...
def test_reconnect_verifies_without_creating_another_backup(tmp_path):
    ...
    assert len(_migration_rows(path)) == 6
...
def test_database_newer_than_application_is_refused(tmp_path):
    ...
    conn.execute(
        ...
        "VALUES (7, 'future', ?, '2026-08-08T00:00:00Z', 'future')",
        ("f" * 64,),
    )
    conn.commit()
    conn.close()
...
def test_applied_migration_history_gap_is_refused(tmp_path):
    ...
    conn.execute(
        "INSERT INTO schema_migrations "
        "(version, name, checksum, applied_ts, app_version) "
        "VALUES (8, 'gap', ?, '2026-08-08T00:00:00Z', 'test')",
        ("f" * 64,),
    )
    conn.commit()
    conn.close()
...
def test_packaged_queue_migration_upgrades_v1_with_verified_backup(
    ...
    try:
        columns = {row[1] for row in upgraded.execute("PRAGMA table_info(queue_jobs)")}
        ...
        assert (
            upgraded.execute("SELECT available_ts FROM queue_jobs").fetchone()[0]
            == "2026-08-08T00:00:00+00:00"
        )
    finally:
        upgraded.close()

    assert [row["version"] for row in _migration_rows(path)] == [1, 2, 3, 4, 5, 6]
...
@pytest.mark.unit
def test_process_worker_migration_upgrades_v2_with_verified_backup(
    tmp_path, monkeypatch
):
    path = tmp_path / "iic.db"
    packaged = db._load_migrations()
    monkeypatch.setattr(db, "_load_migrations", lambda: packaged[:2])
    db.connect(str(path)).close()
    ...
    try:
        ...
        assert (
            upgraded.execute(
                ...
                "AND name='idx_queue_jobs_blocked'"
            ).fetchone()
            is not None
        )
    finally:
...
@pytest.mark.unit
def test_delivery_outbox_migration_upgrades_v3_with_verified_backup(
    tmp_path, monkeypatch
):
    path = tmp_path / "iic.db"
    packaged = db._load_migrations()
    monkeypatch.setattr(db, "_load_migrations", lambda: packaged[:3])
    legacy = db.connect(str(path))
    legacy.execute(
        "INSERT INTO briefs "
        "(brief_id, mode, scope, generated_ts, content_path, run_ids) "
        "VALUES ('b1', 'event_alert', 'AAPL', '2026-08-08T00:00:00Z', 'b1.md', '[]')"
    )
    legacy.execute(
        "INSERT INTO delivery_queue "
        "(brief_id, channel, mode, brief_payload, body, state, attempt_count, "
        "max_attempts, available_ts, created_ts, updated_ts) VALUES "
        "('b1', 'email', 'event_alert', '{}', 'body', 'queued', 0, 5, "
    ...
    try:
        columns = {
            row[1] for row in upgraded.execute("PRAGMA table_info(delivery_queue)")
        }
        assert {
            "idempotency_key",
            "error_category",
        ...
        row = upgraded.execute(
            "SELECT idempotency_key, state, body FROM delivery_queue"
        ).fetchone()
        assert tuple(row) == ("brief:b1:channel:email", "queued", "body")
        assert (
            upgraded.execute(
                "SELECT 1 FROM sqlite_master WHERE type='table' "
                "AND name='delivery_queue_events'"
            ).fetchone()
            is not None
        )
    finally:
    ...
    backups = sorted((tmp_path / "migration-backups").glob("*.db"))
    assert len(backups) == 1
    assert "pre-v0004" in backups[0].name
    assert Path(f"{backups[0]}.sha256").is_file()
...
""",
    ...
    upgraded.commit()
    assert (
        upgraded.execute("SELECT note FROM migration_probe WHERE id=1").fetchone()[0]
        == "seen;ok"
    )
    upgraded.close()

    assert [row["version"] for row in _migration_rows(path)] == [1, 2, 3, 4, 5, 6, 7]
    backups = sorted((tmp_path / "migration-backups").glob("*.db"))
    assert len(backups) == 1
    assert "pre-v0007" in backups[0].name
...
def test_pending_migration_refuses_foreign_key_corruption(tmp_path, monkeypatch):
    ...
    migrations = db._load_migrations()
    pending = db.Migration.from_sql(7, "safe_probe", "CREATE TABLE safe_probe (id);\n")
    monkeypatch.setattr(db, "_load_migrations", lambda: migrations + (pending,))
